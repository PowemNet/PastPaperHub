...
async function fetchAndIntialiseUserData(facebookUserId) {
  await httpGet(`/api/v1/user/` + facebookUserId)
    .then(res => {
...
function generateJsonForItemSelected() {
  ...
  var itemSelectedText =
    profileCardSelectItem.options[profileCardSelectItem.selectedIndex].text;
  var itemSelectedValue =
    profileCardSelectItem.options[profileCardSelectItem.selectedIndex].value;
...
async function showUniversityCard() {
  ...
  await httpGet(`/api/v1/university/country/` + country.id)
    .then(res => {
...
async function showCourseCard() {
  ...
  await httpGet(`/api/v1/course/university/` + university.id)
    .then(res => {
...
async function updateUserProfileForCard() {
  ...
  if (userHasSelectedItem()) {
    await httpPatch(`/api/v1/user/` + user.id, generateJsonForItemSelected())
      .then(res => {
        setUpProfileCard();
        return JSON.stringify(res);
      })
